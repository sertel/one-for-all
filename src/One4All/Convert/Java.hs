module One4All.Convert.Java where
  ...
  convert (L.Let (L.V v) e ie) =
    let ec =
          case L.convert e of
            ...
            otherwise ->
              error $
              "Java does not support this type of expression in an assigment: " ++
              (show e)
        iec =
          case ie of
        ...
        stmt =
          LocalVars
            ...
            (RefType $
             ClassRefType $
             ClassType
               [(Ident "java", []), (Ident "lang", []), (Ident "Object", [])])
    ...
    let (f, args) = L.collectFnCall e
     ...
     in case f of
          ...
          -- -- it is not possible in Java to immediately call a closure, so we need bind it first
           ->
            error $
            "Invariant broken: Java support requires to lift direct lambda applications, i.e., (\\a -> a) 5 => let id = (\\a -> a) in id 5. Offending call: " ++
            (show lambdaApply)
